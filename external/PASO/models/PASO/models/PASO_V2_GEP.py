...
from utils.layers import convolutional_layer, dense_layer, ContextAttentionLayer_V2
...
class Conv_TransMCA_GEP_V2(nn.Module):
    """
    Convolutional Transformer Multi-Channel Attention model with Gene Expression
    """

    def __init__(self, params, *args, **kwargs):


        super(Conv_TransMCA_GEP_V2, self).__init__(*args, **kwargs)
        ...
        self.gene_attention_size = params.get('gene_attention_size', 64)
        ...
        for layer in range(len(self.molecule_gep_heads)):
            for head in range(self.molecule_gep_heads[layer]):
                ...
                e, a = self.molecule_attention_layers_gep[ind](
                    encoded_smiles[layer], gep.permute(0, 2, 1)
                )
                encodings.append(e)
