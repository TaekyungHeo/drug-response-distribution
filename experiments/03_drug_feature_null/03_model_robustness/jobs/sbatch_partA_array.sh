#!/bin/bash
# Part A: SLURM array — one GPU job per fold (2 conditions each).
# Submit:
#   ARRAY_JOB=$(sbatch --parsable sbatch_partA_array.sh)
#   sbatch --dependency=afterok:$ARRAY_JOB sbatch_partA_aggregate.sh
#
# Runtime per fold: ≈ 2 h (2 conditions × 200 epochs).
#SBATCH --job-name=03null_pA_fold
...
#SBATCH --time=3:00:00
#SBATCH --array=0-9
#SBATCH --output=experiments/03_drug_feature_null/03_model_robustness/logs/sbatch_partA_fold%a_%j.log
